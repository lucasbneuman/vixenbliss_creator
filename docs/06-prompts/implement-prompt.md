# Prompt Base - Implementacion

Usar esta plantilla luego de aprobar un plan:

```text
Implementa la siguiente tarea segun el plan aprobado.

Tarea: <ID y titulo>
Plan aprobado: <resumen o enlace>
Contexto clave: <archivos y contratos relevantes>
Restricciones: <lo que no debe tocar>

Instrucciones:
- ejecuta solo el alcance aprobado
- manten el cambio acotado y trazable
- actualiza documentacion si el cambio la afecta
- verifica el resultado con checks relevantes
- resume al final que cambiaste, como lo validaste y que riesgo queda abierto
```
