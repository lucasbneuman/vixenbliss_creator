...
if [ ! -d "${COMFYUI_CUSTOM_NODES_DIR}/ComfyUI-IPAdapter-Flux" ]; then
  echo "ComfyUI runtime is incomplete: IPAdapter Flux was not baked into the image" >&2
  exit 1
fi
...
if [ ! -f "/opt/runpod-s1-image-serverless/workflows/${COMFYUI_WORKFLOW_IMAGE_ID}.json" ]; then
