...
COMFYUI_IP_ADAPTER_MODEL = os.getenv("COMFYUI_IP_ADAPTER_MODEL", "flux-ipadapter-face.safetensors")
...
def _assert_required_runtime_inputs(job_input: dict) -> None:
    paths = _required_runtime_paths(job_input)
    missing = [(key, path.name) for key, path in paths.items() if not path.exists()]
    if missing:
...
def _build_workflow_payload(job_input: dict) -> dict:
    ...
    ip_adapter_weight = float(job_input.get("ip_adapter_weight", 0.85))
    ...
    workflow["face_detailer"]["inputs"]["denoise"] = float(
        job_input.get("inpaint_strength", workflow["face_detailer"]["inputs"]["denoise"])
    )
